...
import {
  startCategoryUploaderLoader,
  doUploadCategory,
  doRemoveCategory,
} from "../../../../actions/siteadmin/manageCategoryImage";
//Message
...
class Dropzone extends Component {
  static propTypes = {
    doUploadCategory: PropTypes.any.isRequired,
    doRemoveCategory: PropTypes.any.isRequired,
    startCategoryUploaderLoader: PropTypes.any.isRequired,
  };
  ...
  async success(file, fromServer) {
    const { doUploadCategory, data, change } = this.props;
    let fileName = fromServer.file.filename;
    let oldPicture = data.image != null ? data.image : null;
    let filePath = fromServer.file.path;
    let image = fileName;
    doUploadCategory(image, filePath, oldPicture, data.id);
    await change("EditCategory", "image", fileName);
  }
  ...
  addedfile(file, fromServer) {
    ...
    if (file && file.size > 1024 * 1024 * maxUploadSize) {
      toastr.error(
        ...
        "Try again with a smaller sized image"
      );
