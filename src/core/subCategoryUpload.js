...
const subCategoryUpload = (app) => {
  ...
  app.post(
    ...
    async (req, res) => {
      let filePath = subCategoryuploadDir;
    ...
    async (req, res, next) => {
      ...
      const smallImage = await new Promise((resolve, reject) => {
        sharp(file.path)
          ...
          .toFile(subCategoryuploadDir + "small_" + file.filename, function(
            err
          ) {
            if (file) {
              resolve(file);
      ...
      if (file.mimetype === "image/jpeg") {
        ...
        const mediumImage = await new Promise((resolve, reject) => {
          sharp(file.path)
            ...
            .jpeg({ quality: 50 })
            //.crop(sharp.strategy.entropy)
            .toFile(subCategoryuploadDir + "medium_" + file.filename, function(
              err
            ) {
              if (file) {
                resolve(file);
