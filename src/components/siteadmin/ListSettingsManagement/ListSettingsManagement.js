...
class ListSettingsManagement extends React.Component {
      ...
      <div>
        ...
        >
          ...
          >
            ...
            {listSettingsData &&
              listSettingsData.length > 0 &&
              listSettingsData.map(function(item, key) {
                let status =
                  item.isEnable == 1
                ...
                return (
                  <Tr key={key}>
                    <Td
                      data-label={formatMessage(messages.settingsIDLabel)}
                      column={formatMessage(messages.settingsIDLabel)}
                      data={item.id}
                    />
                    <Td
                      data-label={formatMessage(messages.addItemNew)}
                      column={formatMessage(messages.addItemNew)}
                      data={item.itemName}
                    />
                    <Td
                    ...
                    />
                    <Td
                      data-label={formatMessage(messages.operationLabel)}
                      column={formatMessage(messages.operationLabel)}
                    >
                      <Button
                        className={cx(bt.btnPrimaryBorder, "textCenterAdmin")}
                        onClick={() =>
                          openListSettingsModal(item, "EditListSettingsForm")
                        }
                      >
        ...
        {listSettingsData && listSettingsData.length > 0 && (
          <div>
            <CustomPagination
              total={count}
              currentPage={page}
  ...
  render() {
    ...
    if (!loading && adminListSettings) {
      status =
        adminListSettings.getAllAdminListSettings &&
        adminListSettings.getAllAdminListSettings.status;
      if (status === 200) {
        listSettingsTypeData =
          adminListSettings.getAllAdminListSettings.listSettingsTypeData;
        listSettingsData =
          adminListSettings.getAllAdminListSettings.listSettingsData;
          ...
          <div className={cx(s.pagecontentWrapper, "pagecontentAR")}>
            <div>
              <h1 className={s.headerTitle}>
                {listSettingsTypeData.typeLabel}
              </h1>
              {listSettingsTypeData.fieldType === "numberType" &&
