import { coverPhotouploadDir } from "../config";
...
var storage = multer.diskStorage({
  destination: coverPhotouploadDir,
...
const coverPhotoUpload = (app) => {
  app.use(bodyParser.urlencoded({ extended: true }));
  app.use(bodyParser.json());

  app.post(
    "/removeCoverPhotoFile",
    ...
    async (req, res) => {
      let filePath = coverPhotouploadDir;
    ...
    async (req, res, next) => {
      ...
      const smallImage = await new Promise((resolve, reject) => {
        sharp(file.path)
          ...
          .toFile(coverPhotouploadDir + "small_" + file.filename, function(
            err
          ) {
            if (file) {
              resolve(file);
      ...
      if (file.mimetype === "image/jpeg") {
        ...
        const mediumImage = await new Promise((resolve, reject) => {
          sharp(file.path)
            ...
            .jpeg({ quality: 50 })
            //.crop(sharp.strategy.entropy)
            .toFile(coverPhotouploadDir + "medium_" + file.filename, function(
              err
            ) {
              if (file) {
                resolve(file);
