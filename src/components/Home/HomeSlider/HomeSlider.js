    ...
    if (
      ...
      (data.length > 4 ||
        (typeof window !== "undefined" &&
          ((window &&
            window.matchMedia("(max-width: 768px) and (min-width: 640px)")
              .matches &&
            data.length > 3) ||
      ...
      <Grid fluid>
        <Row>
          <Col
            ...
            md={12}
            lg={12}
            className={cx("noPadding", "homeSliderPaading", "homeSwiper")}
          >
            {!load && (
              <div>
                <Loader type="text" />
            ...
            {load && (
              <Swiper
                {...params}
                rtl={isRTL(locale)}
                className={cx("row homeSlickSlider", s.noMargin)}
              >
                {data &&
                  data.length > 0 &&
                  data.map((item, index) => {
                    if (item.listPhotos.length > 0) {
                      return (
                        <div
                          className={cx("col-md-12 col-sm-12 col-xs-12")}
                          key={index}
                        >
