...
// Style
import { Row, FormGroup, Col, Panel } from "react-bootstrap";
            ...
            <Panel className={cx(s.panelHeader, s.bannerPadding)}>
              <form onSubmit={handleSubmit(submit)}>
                ...
                <div className={cx("row")}>
                  {homeBannerImages &&
                    homeBannerImages.data.length > 0 &&
                    homeBannerImages.data.map((item, key) => {
                      return (
                        <div
                          key={key}
                          className={cx(
                            "col-lg-4 col-md-6 col-sm-6 col-xs-12 text-center",
                            s.space3
                          )}
                        >
                          <div className={s.listPhotoMedia}>
                            ...
                            <a
                              ...
                              onClick={() =>
                                deleteHomeBanner(item.id, item.name)
                              }
                              className={cx(s.bannerDelete, "trashIconNewRTL")}
                            >
...
export default injectIntl(
  withStyles(s, bt)(connect(mapState, mapDispatch)(HomeBannerForm))
);
