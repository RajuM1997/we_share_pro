...
class ListingDetails extends React.Component {
    ...
    try {
      ...
      if (dynamicFields && typeof dynamicFields === "object") {
        getPageFieldBySubCategory(subCategoryId)
          .then((pageData) => {
            getFieldsBySubCategory(subCategoryId)
              .then((fieldsData) => {
  ...
  render() {
    ...
    const { details, isHost, urlParameters, contactHostOpen } = this.props;
    const data = {};
    return (
            ...
            <div className={cx(s.spaceTop3, s.space4)}>
              <p>
                <a
                  ...
                  onClick={() => contactHostOpen(details.id, urlParameters)}
                >
          ...
          <div className={cx(s.horizontalLineThrough)}>
            <Row>
              <Col xs={12} sm={12} md={12} lg={12} className={cx(s.space1)}>
                <p className={s.textMutedNew}>{ele?.pageTitle}</p>
              </Col>
                    ...
                    {ele?.fields?.map((field) => (
                      <p className={s.splitList}>
                        <span className={cx(s.text)}>
                          {field?.fieldTitle} : <strong>{field?.values}</strong>
                        </span>
                      </p>
                    ))}
                  </Col>
