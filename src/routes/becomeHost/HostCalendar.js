...
class HostCalendar extends Component {
  ...
  render() {
    ...
    isAdminCurrency =
      availableCurrencies &&
      availableCurrencies.find((o) => o.isBaseCurrency == true);
    ...
    let coverImage =
      stepTwoDetails &&
      stepTwoDetails.listPhotos.find((o) => o.id == coverPhoto);
    ...
    if (coverImage) {
      ...
      !coverImage &&
      stepTwoDetails &&
      stepTwoDetails.listPhotos &&
      stepTwoDetails.listPhotos.length > 0
    ) {
      showImage =
        path +
        (stepTwoDetails &&
          stepTwoDetails.listPhotos &&
          stepTwoDetails.listPhotos[0].name);
    } else {
      ...
      <div className={cx(s.landingContainer, s.fullWidthCalendar)}>
        ...
        <div className={s.lableWeight}>
          <p className={cx(s.bookedWidth)}>
            <span className={cx(s.notAvailableColor, "availableRTl")}></span>
          ...
          <p className={s.calenderColorText}>
            <span className={cx(s.availableColor, "availableRTl")}></span>
...
export default injectIntl(
  withStyles(s, bt)(connect(mapState, mapDispatch)(HostCalendar))
);
